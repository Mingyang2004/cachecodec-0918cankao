...
from rosetta.train.dataset_adapters import ConcatDataCollator, ConcatDualPromptDataset, ChatDataset, AlignedChatDataset, RosettaDataCollator, create_dataset, BaselineDataCollator, BaselineChatDataset
...
from rosetta.model.jcb import JCBC2CProjector
from rosetta.model.split_channel_fusion import SplitChannelFusionC2CProjector
...
from rosetta.utils.paths import expand_external_paths
...
torch.autograd.set_detect_anomaly(
    os.environ.get("CACHECODEC_DETECT_ANOMALY", "0") == "1"
)


class LossConvergenceTracker:
    """Detect a plateau in optimizer-step losses using a bounded history."""

    def __init__(self, window_size: int = 20, patience: int = 3, min_delta: float = 1e-3):
        if window_size < 2 or patience < 1 or min_delta < 0:
            raise ValueError("Invalid loss convergence settings.")
...
def remove_previous_checkpoints(output_dir: str, keep_dir: str) -> None:
    """Keep only the newly written periodic checkpoint when configured."""
    ...
    for name in os.listdir(root):
        ...
        if candidate != keep and os.path.isdir(candidate):
...
def unfreeze_projectors(rosetta_model: RosettaModel):
    ...
    for projector in rosetta_model.projector_list:
        for param in projector.parameters():
            param.requires_grad = True
    for codec in getattr(rosetta_model, "cache_codec_list", []):
...
def detect_training_mode(model_config: Dict[str, Any]) -> str:
    ...
    if "baseline_model" in model_config and "base_model" not in model_config:
        ...
        ("base_model" in model_config and "teacher_model" in model_config)
        or ("receiver_model" in model_config and "sharer_model" in model_config)
    ):
        return "rosetta"
    else:
        raise ValueError("Invalid model configuration. Provide either 'baseline_model' for baseline training "
                       "or receiver/sharer (legacy base/teacher) models for Rosetta training.")
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    return expand_external_paths(config)
...
def build_training_metrics_record(
    *,
    phase: str,
    step: int,
    elapsed_seconds: float,
    train_loss: Optional[float] = None,
    task_ce_loss: Optional[float] = None,
    ...
    """Build a JSON-serializable per-step record for convergence analysis."""
    record: Dict[str, Any] = {
        "split": split,
        "phase": phase,
        "step": int(step),
    ...
    scalar_fields = {
        ...
        "task_ce_loss": task_ce_loss,
        "learning_rate": learning_rate,
        "grad_norm": grad_norm,
        "epoch": epoch,
        "eval_loss": eval_loss,
    }
    record.update({key: float(value) for key, value in scalar_fields.items() if value is not None})
    if quant_metrics:
...
def write_metrics_record(handle: Any, record: Dict[str, Any]) -> None:
    """Append and flush one JSONL record so progress survives interruptions."""
...
def load_resume_checkpoint(
    ...
    if training_mode == "rosetta":
        ...
        for i, codec in enumerate(getattr(model_ref, "cache_codec_list", [])):
            codec_path = os.path.join(checkpoint_dir, f"cache_codec_{i}.pt")
            if not os.path.isfile(codec_path):
                raise FileNotFoundError(f"Missing cache codec checkpoint: {codec_path}")
            codec.load_state_dict(torch.load(codec_path, map_location=device))
...
def setup_models(model_config: Dict[str, Any], training_mode: str, device: str = "cuda", dtype: torch.dtype = torch.bfloat16):
    ...
    if training_mode == "baseline":
        ...
        # Prefer explicit role names while retaining the historical base/teacher
        # keys so old checkpoints and recipes remain loadable.
        base_model_name = model_config.get("receiver_model") or model_config["base_model"]
        teacher_model_name = model_config.get("sharer_model") or model_config["teacher_model"]
        normalized_config = dict(model_config)
        normalized_config["base_model"] = base_model_name
        normalized_config["teacher_model"] = teacher_model_name
        normalized_config["base_model_local_dir"] = model_config.get(
            "receiver_model_local_dir", model_config.get("base_model_local_dir")
        )
        normalized_config["teacher_model_local_dir"] = model_config.get(
            "sharer_model_local_dir", model_config.get("teacher_model_local_dir")
        )
        base_model_load_path = resolve_model_load_path(
            normalized_config, "base_model", "base_model_local_dir"
        )
        teacher_model_load_path = resolve_model_load_path(
            normalized_config, "teacher_model", "teacher_model_local_dir"
        )
        ...
        if str(model_config.get("training_stage", "")).lower() == "jcb" and bool(
            (model_config.get("adaptive_quant_table") or {}).get("enabled", False)
        ):
            raise ValueError(
                "JCB training does not support adaptive quantization or rate loss; "
                "set model.adaptive_quant_table.enabled=false."
            )

        if cache_alignment == "concat":
            if fusion_type != "original":
                raise ValueError("concat prefix requires model.fusion_type='original'.")
            ...
            if concat_projector_type not in {"raw", "direct", "lcf_first", "lcf_projected_kv", "jcb", "split_channel_fusion"}:
                raise ValueError(
                    "model.concat_projector.type must be 'raw', 'lcf_first', "
            ...
            for _ in range(slm_num_layers):
                ...
                if concat_projector_type in {"raw", "direct"}:
                    projector = create_projector(
                        "DirectConcatProjector",
                        hidden_dim=int(concat_projector_config.get("hidden_dim", 1024)),
                        num_layers=int(concat_projector_config.get("num_layers", 2)),
                        dropout=float(concat_projector_config.get("dropout", 0.0)),
                        **common_projector_args,
                    )
                elif concat_projector_type == "split_channel_fusion":
                    projector = create_projector(
                        "SplitChannelFusionProjector",
                        key_latent_dim=int(concat_projector_config.get("key_latent_dim", 64)),
                        value_latent_dim=int(concat_projector_config.get("value_latent_dim", 64)),
                        **common_projector_args,
                    )
                elif concat_projector_type in {"lcf_projected_kv", "jcb"}:
                    projector = create_projector(
                        "JCBProjector" if concat_projector_type == "jcb" else "LCFProjectedKVProjector",
        ...
        cache_codec_list = []
        cache_codec_cfg = dict(model_config.get("cache_codec") or {})
        cache_codec_enabled = bool(cache_codec_cfg.get("enabled", False))
        if cache_codec_enabled and bool(
            (model_config.get("adaptive_quant_table") or {}).get("enabled", False)
        ):
            raise ValueError(
                "The fixed JCB cache codec cannot be combined with adaptive quantization or rate loss."
            )
        if cache_codec_enabled:
            if cache_alignment != "fuser":
                raise ValueError("model.cache_codec is currently a C2C/fuser-only Sharer cache codec.")
            if fusion_type != "original":
                raise ValueError("model.cache_codec requires fusion_type='original' so the original C2C fuser remains active.")
            cache_codec_type = str(cache_codec_cfg.get("type", "jcb")).lower()
            if cache_codec_type not in {"jcb", "split_channel_fusion"}:
                raise ValueError("model.cache_codec.type must be 'jcb' or 'split_channel_fusion'.")
            for _ in range(slm_num_layers):
                cache_codec_list.append(
                    (SplitChannelFusionC2CProjector if cache_codec_type == "split_channel_fusion" else JCBC2CProjector)(
                        sharer_num_kv_heads=teacher_num_heads,
                        sharer_head_dim=teacher_dim,
                        receiver_num_kv_heads=base_num_heads,
                        receiver_head_dim=base_dim,
        ...
        # A fuser-side cache codec reconstructs Sharer KV before the original
        # C2C fuser.  It therefore needs its own codec modules *and* the
        # regular fuser projectors below.  This must be a separate branch from
        # cache_codec_enabled rather than an ``elif`` of it.
        ...
        if (
            ...
            and concat_projector_type in {"raw", "direct"}
            and adaptive_quant_config.enabled
        ):
            raise ValueError("Raw concat does not support adaptive quantization.")
        adaptive_quant_table = build_adaptive_quantizer(
            ...
            concat_projector_type=concat_projector_type,
        )
        if adaptive_quant_table is not None:
            adaptive_quant_table = adaptive_quant_table.to(device)

        dct_qat_cfg = dict(model_config.get("dct_qat") or {})
        dct_qat_enabled = bool(dct_qat_cfg.get("enabled", False))
        if dct_qat_enabled:
            if adaptive_quant_config.enabled:
                raise ValueError(
                    "DCT QAT cannot be combined with adaptive quantization or rate loss."
                )
            if cache_alignment == "concat" and concat_projector_type not in {
                "jcb",
                "lcf_projected_kv",
                "split_channel_fusion",
            }:
                raise ValueError(
                    "Concat DCT QAT requires concat_projector.type='jcb', 'lcf_projected_kv', or 'split_channel_fusion'."
                )
            if cache_alignment == "fuser" and not cache_codec_enabled:
                raise ValueError(
                    "Fusion DCT QAT requires model.cache_codec.enabled=true."
                )
            dct_qat = DCTFakeQuantizer(
                beta=float(dct_qat_cfg.get("beta", 1.0)),
                low=float(dct_qat_cfg.get("low", 1.0)),
                high=float(dct_qat_cfg.get("high", 8.0)),
                straight_through=bool(dct_qat_cfg.get("straight_through", True)),
            ).to(device)
        else:
            dct_qat = None
        ...
        aligner = None
        if cache_alignment == "fuser" and bool(model_config.get("is_do_alignment", False)):
            strategy = str(model_config.get("alignment_strategy", "first")).lower()
...
def train_step(model: nn.Module, batch: Dict[str, Any], tokenizer: AutoTokenizer, max_length: int, device: str, training_mode: str):
    ...
    if training_mode == "baseline":
        ...
        labels = batch["labels"].to(device)
        kv_cache_index = (
            [x.to(device) for x in batch["kv_cache_index"]]
            if "kv_cache_index" in batch
            else None
        )
...
def valid_label_token_count(labels: Any) -> int:
    """Count supervised response tokens (labels not masked with -100)."""
    if labels is None:
        return 0
    if isinstance(labels, (list, tuple)):
        return sum(valid_label_token_count(item) for item in labels)
...
def evaluate_model(model: nn.Module, eval_loader: DataLoader, tokenizer: AutoTokenizer, max_length: int, device: str, training_mode: str) -> float:
    """Evaluate response-token CE weighted by the number of valid labels."""
    model.eval()
    eval_loss_total = 0.0
    ...
    with torch.no_grad():
        for eval_batch in eval_loader:
            ...
            valid_tokens = valid_label_token_count(eval_batch.get("labels"))
            if valid_tokens <= 0:
                continue
            eval_loss_total += eval_loss.item() * valid_tokens
            valid_tokens_total += valid_tokens
    
    avg_eval_loss = eval_loss_total / valid_tokens_total if valid_tokens_total > 0 else 0.0
...
def main():
    ...
    if is_main_process:
        ...
        logger.info("Per-step metrics will be saved to: %s", metrics_path)
    ...
    if training_mode == "baseline":
        ...
        if "projector" in freeze_config:
            ...
            for codec in getattr(model, "cache_codec_list", []):
                freeze_model(codec)
        else:
    ...
    if training_mode == "baseline":
        ...
        if model_config.get("cache_alignment") == "concat":
            full_dataset = ConcatDualPromptDataset(
                instruct_ds,
    ...
    if training_mode == "baseline":
        ...
        if model_config.get("cache_alignment") == "concat":
            collator = ConcatDataCollator(main_tokenizer, llm_tokenizer)
        else:
            collator = RosettaDataCollator(
                ...
                max_length=training_config.get("max_length", 2048),
                aligner=aligner,
                do_alignment=aligner is not None,
            )
    else:
        raise ValueError(f"Invalid training mode: {training_mode}")
    ...
    # Training loop
    # ------------------------------------------------------------------
    early_stopping_config = dict(training_config.get("early_stopping") or {})
    save_interval = max(1, int(output_config.get("save_steps", 100)))
    eval_interval = max(1, int(output_config.get("eval_steps", save_interval)))
    early_stopping_enabled = bool(early_stopping_config.get("enabled", False))
    convergence_tracker = (
        LossConvergenceTracker(
            window_size=int(early_stopping_config.get("window_size", 20)),
    ...
    for epoch in range(start_epoch, training_config["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if did_step and convergence_tracker is not None:
                local_stop = convergence_tracker.update(avg_window_loss)
                stop_training = broadcast_decision_from_rank0(
                    local_stop, distributed, device, rank
                )
                if stop_training and is_main_process:
                    logger.info(
                        "Stopping early at optimizer step %s: loss plateau detected "
                        "(window=%s, patience=%s, min_delta=%s).",
                        global_step,
                        convergence_tracker.window_size,
                        convergence_tracker.patience,
                        convergence_tracker.min_delta,
                    )
            ...
            if is_main_process and did_step:
                ...
                if training_mode == "rosetta":
                    ...
                    dct_qat_metrics = model_to_use.last_dct_qat_stats
                    if dct_qat_metrics is not None:
                        train_metrics.update(
                            {
                                f"dct_qat_{key}": value
                                for key, value in dct_qat_metrics.items()
                ...
                if is_main_process:
                    quant_record_metrics = dict(quant_metrics or {}) if training_mode == "rosetta" else {}
                    if training_mode == "rosetta" and dct_qat_metrics is not None:
                        quant_record_metrics.update(
                            {
                                f"dct_qat_{key}": value
                                for key, value in dct_qat_metrics.items()
                                if value is not None and isinstance(value, (int, float, bool))
                    ...
                    task_ce_loss = quant_record_metrics.pop("task_ce_loss", None)
                    write_metrics_record(
                        metrics_handle,
                        build_training_metrics_record(
                            phase=str(model_config.get("training_stage", training_mode)),
                            step=global_step,
                            elapsed_seconds=time.perf_counter() - metrics_start_time,
                            train_loss=avg_window_loss,
                            task_ce_loss=task_ce_loss if task_ce_loss is not None else avg_window_loss,
                            learning_rate=scheduler.get_last_lr()[0],
                            grad_norm=grad_norm_value,
                            epoch=fractional_epoch,
                            quant_metrics=quant_record_metrics,
                        ),
                    )
            ...
            if did_step:
                ...
                want_save = (global_step % save_interval == 0)
                ...
                if want_save:
                    if is_main_process:
                        ...
                        if bool(output_config.get("keep_only_latest_checkpoint", False)):
                            remove_previous_checkpoints(timestamped_output_dir, checkpoint_dir)
                        ...
                        if training_mode == "baseline":
                            ...
                            for i, codec in enumerate(getattr(base_model_ref, "cache_codec_list", [])):
                                torch.save(codec.state_dict(), os.path.join(checkpoint_dir, f"cache_codec_{i}.pt"))
                                save_projector(codec, os.path.join(checkpoint_dir, f"cache_codec_{i}.json"))
                ...
                want_eval = (global_step % eval_interval == 0)
        ...
        avg_epoch_loss = epoch_loss / max(1, (batch_idx + 1 - skip_batches))
    ...
    if is_main_process:
        ...
        if training_mode == "baseline":
            ...
            for i, codec in enumerate(getattr(base_model_ref, "cache_codec_list", [])):
                torch.save(codec.state_dict(), os.path.join(final_dir, f"cache_codec_{i}.pt"))
                save_projector(codec, os.path.join(final_dir, f"cache_codec_{i}.json"))
            if base_model_ref.adaptive_quant_table is not None:
                torch.save(
                    base_model_ref.adaptive_quant_table.state_dict(),
