...
class TransportStats:
    ...
    link_model_seconds: float
    socket_runtime_seconds: float
    ...
    @property
    def transport_wall_seconds(self) -> float:
        return self.serialize_seconds + self.transmit_seconds + self.deserialize_seconds
...
class SocketPairTransport:
    ...
    def roundtrip(self, payload: Any) -> Any:
        ...
        frame_bytes = payload_bytes + _LENGTH_HEADER.size
        if payload_bytes > self.max_payload_bytes:
            raise ValueError(f"CacheJPEG payload has {payload_bytes} bytes, exceeding max_payload_bytes={self.max_payload_bytes}.")
        link_model_seconds = max(0.0, self.fixed_latency_ms / 1000.0)
        if self.bandwidth_bytes_per_sec is not None:
            if self.bandwidth_bytes_per_sec <= 0:
                raise ValueError("transport.bandwidth_bytes_per_sec must be positive.")
            link_model_seconds += frame_bytes / self.bandwidth_bytes_per_sec
        ...
        def send() -> None:
            try:
                if link_model_seconds:
                    time.sleep(link_model_seconds)
        ...
        self.last_stats = TransportStats(
            ...
            link_model_seconds=link_model_seconds,
            socket_runtime_seconds=max(0.0, transmit_seconds - link_model_seconds),
...
class DirectTransport:
    ...
    def roundtrip(self, payload: Any) -> Any:
        ...
        self.last_stats = TransportStats(
            payload_bytes=len(blob),
            frame_bytes=len(blob) + _LENGTH_HEADER.size,
            serialize_seconds=serialize_seconds,
            link_model_seconds=0.0,
            socket_runtime_seconds=0.0,
