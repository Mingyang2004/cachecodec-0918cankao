...
@dataclass(frozen=True)
class _RoutedLayerTask:
    route: tuple[int, int, int]
    task: PreRopeLayerTask
...
class ConcatLayerPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.routes_by_source: dict[int, list[tuple[int, int, int]]] = {}
        for route in routing.routes:
            self.routes_by_source.setdefault(route[1], []).append(route)
    ...
    def submit(self, task: PreRopeLayerTask) -> None:
        routes = self.routes_by_source.get(task.layer_idx, ())
        for route in routes:
            ...
            if self.error is not None:
                ...
                raise RuntimeError("Concat layer pipeline failed.") from self.error
            self.encode_queue.put(_RoutedLayerTask(route, task))
                ...
                try:
                    ...
                    if self.transport is not None and self.transport.last_stats is not None:
                        with self.lock:
                            ...
                            self.payload_bytes += int(self.transport.last_stats.frame_bytes)
                    else:
                        with self.lock:
                            self.payload_bytes += len(serialize_payload(encoded.payload))
    ...
    def aggregate_transport_stats(self) -> TransportStats | None:
        ...
        return TransportStats(
            ...
            frame_bytes=sum(item.frame_bytes for item in self.transport_stats),
            serialize_seconds=sum(item.serialize_seconds for item in self.transport_stats),
            link_model_seconds=sum(item.link_model_seconds for item in self.transport_stats),
            socket_runtime_seconds=sum(item.socket_runtime_seconds for item in self.transport_stats),
