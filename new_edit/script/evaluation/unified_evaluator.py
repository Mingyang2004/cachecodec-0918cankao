...
from rosetta.baseline.t2t import load_t2t_model
...
class UnifiedEvaluator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.is_t2t_model = self.model_name_normalized in {"t2t", "token_to_token"}
        if self.is_t2t_model and str(self.eval_config.get("answer_method", "generate")).lower() != "generate":
            raise ValueError("T2T evaluation currently supports answer_method='generate' only.")
        ...
        if isinstance(rosetta_cfg, dict) and rosetta_cfg:
            receiver_model = rosetta_cfg.get("receiver_model", rosetta_cfg.get("base_model"))
            sharer_model = rosetta_cfg.get("sharer_model", rosetta_cfg.get("teacher_model"))
            resolved_receiver = self._resolve_model_ref(
                receiver_model,
                ("receiver_model_path", "receiver_model_local_dir", "base_model_path", "base_model_local_dir"),
                rosetta_cfg,
            )
            resolved_sharer = self._resolve_model_ref(
                sharer_model,
                ("sharer_model_path", "sharer_model_local_dir", "teacher_model_path", "teacher_model_local_dir"),
                rosetta_cfg,
            )
            self.model_config["rosetta_config"]["base_model"] = resolved_receiver
            self.model_config["rosetta_config"]["teacher_model"] = resolved_sharer
            self.model_config["rosetta_config"]["receiver_model"] = resolved_receiver
            self.model_config["rosetta_config"]["sharer_model"] = resolved_sharer
    ...
    def prepare_model_inputs(self, prompt: str, tokenizer, device: torch.device,
        ...
        if not use_dual_tokenizers:
            ...
            if model_type == "t2t":
                outputs["inputs"]["t2t_prompt_text"] = prompt
        ...
        if use_dual_tokenizers:
            ...
            rosetta_cfg = self.model_config.get("rosetta_config", {})
            cachejpeg_cfg = self.model_config.get("cachejpeg_rosetta_config", {})
            do_c2c_alignment = (
                model_type == "cachejpeg_rosetta"
                and str(cachejpeg_cfg.get("cache_alignment", "fuser")).lower() == "fuser"
                and bool(rosetta_cfg.get("is_do_alignment", False))
            )
            receiver_text = tokenizer.apply_chat_template(
                messages,
                tokenize=False,
                add_generation_prompt=add_generation_prompt,
                enable_thinking=False,
            )
            sharer_text = llm_tokenizer.apply_chat_template(
                messages,
                tokenize=False,
                add_generation_prompt=add_generation_prompt,
                enable_thinking=False,
            ...
            if do_c2c_alignment:
                aligner = TokenAligner(
                    ...
                    strategy=AlignmentStrategy(
                        str(rosetta_cfg.get("alignment_strategy", "first")).lower()
                    ),
                )
                details = aligner.align_chat_messages(
                    messages, add_generation_prompt=add_generation_prompt, return_details=True
                )
                receiver_ids = torch.tensor(details["slm_ids_padded"], dtype=torch.long, device=device).unsqueeze(0)
                sharer_ids = torch.tensor(details["llm_ids_padded"], dtype=torch.long, device=device).unsqueeze(0)
                receiver_mask = (~torch.tensor(details["slm_padding_mask"], dtype=torch.bool, device=device)).float().unsqueeze(0)
                sharer_mask = (~torch.tensor(details["llm_padding_mask"], dtype=torch.bool, device=device)).float().unsqueeze(0)
            else:
                receiver_tokens = tokenizer(receiver_text, return_tensors="pt").to(device)
                sharer_tokens = llm_tokenizer(sharer_text, return_tensors="pt").to(device)
                receiver_ids, sharer_ids = receiver_tokens["input_ids"], sharer_tokens["input_ids"]
                receiver_mask, sharer_mask = receiver_tokens["attention_mask"], sharer_tokens["attention_mask"]
                if (
                    model_type == "cachejpeg_rosetta"
                    and str(cachejpeg_cfg.get("cache_alignment", "fuser")).lower() == "fuser"
                    and not do_c2c_alignment
                ):
                    sharer_ids, sharer_mask = receiver_ids, receiver_mask

            aligned_inputs = {
                "input_ids": [receiver_ids, sharer_ids],
                "attention_mask": [receiver_mask.float(), sharer_mask.float()],
            }
            if model_type == "rosetta":
                receiver_length = receiver_ids.shape[1]
                aligned_inputs["position_ids"] = torch.arange(receiver_length).unsqueeze(0).to(device)
            outputs = {
                "inputs": aligned_inputs,
                "printable_text": (receiver_text, sharer_text),
            }
    ...
    def evaluate_subject(self, subject: str, model, tokenizer, device: torch.device, 
        ...
        if self.dataset_name == "math-500":
            ...
            local_jsonl_file = self.eval_config.get("local_jsonl_file")
            if local_jsonl_file:
                local_jsonl_file = os.path.abspath(os.path.expanduser(str(local_jsonl_file)))
                if not os.path.isfile(local_jsonl_file):
                    raise FileNotFoundError(f"ARC-Challenge local_jsonl_file not found: {local_jsonl_file}")
                local_data = load_dataset("json", data_files={"test": local_jsonl_file}, split="test")

                def _normalize_arc_example(example: Dict[str, Any]) -> Dict[str, Any]:
                    endings = list(example.get("endings") or [])
                    return {
                        "question": str(example.get("ctx", "")),
                        "choices": {"text": [str(choice) for choice in endings], "label": [chr(ord("A") + index) for index in range(len(endings))]},
                        "answerKey": str(example.get("answer", "")).upper(),
                        "subject": example.get("subject", "ARC-Challenge"),
                    }

                test_data = local_data.map(_normalize_arc_example).filter(
                    lambda example: example.get("subject") == base_subset
                )
                dataset = None
            else:
                dataset = load_dataset(self.dataset_config["dataset_name"], base_subset)
            ...
            if local_jsonl_file:
                local_jsonl_file = os.path.abspath(os.path.expanduser(str(local_jsonl_file)))
                if not os.path.isfile(local_jsonl_file):
                    raise FileNotFoundError(f"C-Eval local_jsonl_file not found: {local_jsonl_file}")
                local_data = load_dataset("json", data_files={"test": local_jsonl_file}, split="test")

                def _normalize_ceval_example(example: Dict[str, Any]) -> Dict[str, Any]:
                    endings = list(example.get("endings") or [])
                    normalized = {
                        "question": str(example.get("ctx", "")),
                        "answer": str(example.get("answer", "")).upper(),
                        "subject": example.get("subject", subject),
                    }
                    normalized.update({
                        chr(ord("A") + index): str(choice)
                        for index, choice in enumerate(endings[:4])
                    })
                    return normalized

                test_data = local_data.map(_normalize_ceval_example).filter(
        ...
        for i in tqdm(sample_indices, desc=f"Evaluating {subject} ({self.eval_config['answer_method']})"):
            try:
                ...
                if model_type in ["two_stage", "two_stage_rosetta"]:
                    ...
                    if self.eval_config["answer_method"] == 'logits':
                        ...
                        if model_type == "t2t":
                            ...
                            input_length = int(
                                getattr(model, "last_receiver_input_length", 0)
                                or prepared["inputs"]["input_ids"].shape[1]
                            )
                            generated_ids = generated_ids[input_length:]
                        elif isinstance(model, RosettaModel):
                            generated_ids = outputs[0]
                ...
                timing_stats = getattr(model, "last_timing_stats", None)
                ...
                if self.eval_config["answer_method"] == 'generate' and input_length is not None and gen_length is not None:
                    ...
                    length_entry = {
                        ...
                        # This outer synchronized generate() measurement is the
                        # authoritative model e2e latency.  Internal component
                        # timing is diagnostic and must not replace it.
                        'model_e2e_ms': (
                            float(latency_ms) if latency_ms is not None else None
                        ),
                    }
                    if isinstance(cachejpeg_stats, dict):
                        ...
                        length_entry.update({
                            ...
                            "quantization_mode": cachejpeg_stats.get("quantization_mode"),
                            "quant_table_low": cachejpeg_stats.get("quant_table_low"),
                            "quant_table_high": cachejpeg_stats.get("quant_table_high"),
                            "scale_mode": cachejpeg_stats.get("scale_mode"),
                            ...
                            "receiver_decode_ms": (
                                float(cachejpeg_stats["receiver_decode_ms"])
                                if cachejpeg_stats.get("receiver_decode_ms") is not None
                                else (
                                    float(cachejpeg_stats["receiver_decode_seconds"]) * 1000.0
                                    if cachejpeg_stats.get("receiver_decode_seconds") is not None
                                    else None
                            ...
                            "sharer_prefill_ms": cachejpeg_stats.get("sharer_prefill_ms"),
                            "receiver_prefill_ms": cachejpeg_stats.get("receiver_prefill_ms"),
                            "receiver_first_token_ms": cachejpeg_stats.get("receiver_first_token_ms"),
                            "fusion_ms": cachejpeg_stats.get("fusion_ms"),
                            "ttft_ms": cachejpeg_stats.get("ttft_ms"),
                        })
                    ...
                    # New schema wins over legacy aliases above.  It keeps
                    # link-model time distinct from local socket runtime and
                    # stores only measured/applicable pipeline stages.
                    if isinstance(timing_stats, dict):
                        length_entry["timing"] = dict(timing_stats)
                        for field, value in timing_stats.items():
                            if field != "timing" and value is not None:
                                length_entry[field] = value
                    t2t_stats = getattr(model, "last_stats", None)
                    if model_type == "t2t" and isinstance(t2t_stats, dict):
                        for field in (
                            "sharer_input_tokens",
                            "sharer_decode_tokens",
                            "bridge_text_length",
                            "receiver_reencoded_tokens",
                        ):
                            if t2t_stats.get(field) is not None:
                                length_entry[field] = int(t2t_stats[field])
                        for field in (
                            "sharer_prefill_ms",
                            "sharer_token_decode_ms",
                            "sharer_decode_ms",
                            "receiver_encode_ms",
                            "receiver_prefill_ms",
                            "receiver_token_decode_ms",
                            "receiver_first_token_ms",
                            "receiver_decode_ms",
                            "ttft_ms",
                            "internal_end_to_end_latency_ms",
                        ):
                            if t2t_stats.get(field) is not None:
                                length_entry[field] = float(t2t_stats[field])
                        if isinstance(t2t_stats.get("timing"), dict):
                            length_entry["timing"] = dict(t2t_stats["timing"])
                            for field, value in t2t_stats["timing"].items():
            ...
            # Save LongBench predictions immediately for post-processing.
                if self.dataset_name == "longbench":
                    ...
                    if isinstance(timing_stats, dict):
                        output_entry["timing"] = dict(timing_stats)
    ...
    def evaluate_on_gpu(self, rank: int, gpu_id: int, subjects: List[str], return_dict):
        ...
        if "two_stage_rosetta" == self.model_config["model_name"].lower():
        ...
        elif self.is_t2t_model:
            model, tokenizer = load_t2t_model(
                self.model_config,
                device=device,
                generation_config=self.generation_config,
            )
            model_type = "t2t"
            llm_tokenizer = getattr(model, "sharer_tokenizer", None)
            print(f"Initialized T2T evaluation pipeline on GPU {gpu_id}")
            ...
            model_type = "cachejpeg_rosetta"
            rosetta_cfg = self.model_config.get("rosetta_config", {})
            llm_tokenizer = getattr(model, "teacher_tokenizer", None)
    ...
    def _compute_timing_statistics(
        ...
        field_names = {
            ...
            "codec_decode_ms": "avg_codec_decode_ms",
            "serialize_ms": "avg_serialize_ms",
            "transmit_ms": "avg_transmit_ms",
            "link_model_ms": "avg_link_model_ms",
            "transport_runtime_ms": "avg_transport_runtime_ms",
            "deserialize_ms": "avg_deserialize_ms",
            "transport_total_ms": "avg_transport_total_ms",
            "transport_wall_ms": "avg_transport_wall_ms",
            "bandwidth_only_transmit_ms": "avg_bandwidth_only_transmit_ms",
            "sender_projection_ms": "avg_sender_projection_ms",
            "gpu_dct_ms": "avg_gpu_dct_ms",
            "gpu_quant_ms": "avg_gpu_quant_ms",
            "d2h_ms": "avg_d2h_ms",
            "entropy_encode_ms": "avg_entropy_encode_ms",
            "entropy_decode_ms": "avg_entropy_decode_ms",
            "h2d_ms": "avg_h2d_ms",
            "gpu_idct_ms": "avg_gpu_idct_ms",
            "receiver_projection_or_fusion_ms": "avg_receiver_projection_or_fusion_ms",
            "receiver_reencode_ms": "avg_receiver_reencode_ms",
            "pipeline_critical_path_ms": "avg_pipeline_critical_path_ms",
            "model_e2e_internal_ms": "avg_model_e2e_internal_ms",
            "ttft_ms": "avg_ttft_ms",
            "sharer_decode_ms": "avg_sharer_decode_ms",
            "sharer_prefill_ms": "avg_sharer_prefill_ms",
            "sharer_token_decode_ms": "avg_sharer_token_decode_ms",
            "receiver_encode_ms": "avg_receiver_encode_ms",
            "receiver_prefill_ms": "avg_receiver_prefill_ms",
            "receiver_first_token_ms": "avg_receiver_first_token_ms",
            "receiver_decode_ms": "avg_receiver_decode_ms",
            "fusion_ms": "avg_fusion_ms",
        }
...
def main():
    ...
    with open(args.config, "r") as f:
        config = expand_external_paths(yaml.safe_load(f))
