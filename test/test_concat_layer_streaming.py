...
def test_concat_pipeline_fans_one_sharer_layer_out_to_multiple_receiver_layers():
    routing = LCFLatentRouting(
        routes=((0, 0, 0), (1, 0, 1), (2, 1, 2)),
        latent_dim=4,
        sequence_length=0,
    )
    pipeline = ConcatLayerPipeline(
    ...
    assert sorted(decoded) == [0, 1, 2]
    assert torch.equal(decoded[0][0], torch.ones(1, 1, 3, 4))
    assert torch.equal(decoded[1][0], torch.ones(1, 1, 3, 4))
    assert torch.equal(decoded[2][0], torch.full((1, 1, 3, 4), 2.0))
    assert finished_routing.routes == routing.routes
    assert sorted(pipeline.layer_timings) == [0, 1, 2]
