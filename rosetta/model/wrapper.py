...
class RosettaModel(nn.Module):
    ...
    """
    def __init__(self, model_list: List[PreTrainedModel], base_model_idx = 0, projector_list: List[Projector] = [], include_response: bool = False, multi_source_fusion_mode: str = "parallel", adaptive_quant_table: Optional[AdaptiveCoefficientQuantizer] = None, cache_alignment: str = "fuser", transport_config: Optional[dict] = None, cache_codec_list: Optional[List[nn.Module]] = None, dct_qat: Optional[nn.Module] = None):
        super().__init__()
        ...
        self.cache_codec_list = nn.ModuleList(cache_codec_list or []).to(device=device, dtype=dtype)
        self.dct_qat = dct_qat.to(device=device) if dct_qat is not None else None
        self.last_dct_qat_stats = None
    ...
    def _apply_source_cache_codec(self, source_model_idx: int, cache: DynamicCache) -> DynamicCache:
        """Reconstruct Sharer KV before the unchanged C2C projector/fuser."""
        if not self.cache_codec_list:
            return cache
        layer_map = self.projector_dict.get(self.base_model_idx, {}).get(source_model_idx, {})
        restored = clone_kv_cache(cache)
        seen_sources = set()
        for target_layer_idx, entry in layer_map.items():
            pairs = entry if isinstance(entry, list) else [entry]
            if not pairs:
                continue
            source_layer_idx, _ = pairs[0]
            source_layer_idx = int(source_layer_idx)
            if source_layer_idx in seen_sources:
                continue
            codec = self.cache_codec_list[min(int(target_layer_idx), len(self.cache_codec_list) - 1)]
            if not hasattr(codec, "decode_source"):
                raise TypeError(
                    "C2C cache codec must expose encode() and decode_source(); "
                    f"got {codec.__class__.__name__}."
                )
            key, value = restored.key_cache[source_layer_idx], restored.value_cache[source_layer_idx]
            key_latent, value_latent = codec.encode((key, value))
            if self.dct_qat is not None:
                key_latent = self.dct_qat(key_latent)
                value_latent = self.dct_qat(value_latent)
                self.last_dct_qat_stats = getattr(self.dct_qat, "last_stats", None)
            restored_key, restored_value = codec.decode_source(key_latent, value_latent)
            restored.key_cache[source_layer_idx] = restored_key.to(device=key.device, dtype=key.dtype)
            restored.value_cache[source_layer_idx] = restored_value.to(device=value.device, dtype=value.dtype)
            seen_sources.add(source_layer_idx)
        return restored
    ...
    def _forward_concat_lcf_first(
        ...
        **kwargs,
        ...
        """Train a concat prefix path with raw, LCF, or JCB transport.
        ...
        from rosetta.model.split_channel_fusion import SplitChannelFusionProjector
        from rosetta.model.projector import DirectConcatProjector

        self.last_dct_qat_stats = None

        if not isinstance(input_ids, list) or not isinstance(attention_mask, list):
            raise ValueError(
                "concat LCF-first training requires separate receiver and sharer "
        ...
        prompt_length = int(sharer_mask[0].long().sum().item())
        if prompt_length <= 0:
            raise ValueError("Concat training requires a non-empty sharer prompt.")
        ...
        for receiver_layer in range(receiver_layers):
            ...
            if not isinstance(
                projector,
                (DirectConcatProjector, LCFFirstProjector, LCFProjectedKVProjector, SplitChannelFusionProjector),
            ):
                raise TypeError(
                    "concat cache_alignment requires DirectConcatProjector, "
                    "LCFFirstProjector, LCFProjectedKVProjector, or SplitChannelFusionProjector modules."
                )
            ...
            if isinstance(projector, DirectConcatProjector):
                key_latent, value_latent = source_kv
            elif isinstance(projector, (LCFProjectedKVProjector, SplitChannelFusionProjector)):
            ...
            if self.dct_qat is not None:
                if isinstance(projector, DirectConcatProjector):
                    raise ValueError("DCT QAT requires a JCB/latent concat projector, not raw concat.")
                key_latent = self.dct_qat(key_latent)
                value_latent = self.dct_qat(value_latent)
                self.last_dct_qat_stats = getattr(self.dct_qat, "last_stats", None)
        ...
        for receiver_layer, (projector, key_latent, value_latent) in enumerate(encoded_layers):
            ...
            if isinstance(projector, DirectConcatProjector):
                receiver_key, receiver_value = projector.decode_source(
        ...
        if quant_result is not None:
            task_loss = output.loss.detach() if output.loss is not None else None
            ...
            self.last_adaptive_quant_table_metrics = {
                "task_ce_loss": float(task_loss.item()) if task_loss is not None else None,
                "alpha_by_layer_kv": quant_result.alpha.detach().float().cpu().tolist(),
                "table_indices_by_layer_kv": quant_result.table_indices.detach().cpu().tolist(),
        ...
        **kwargs,
        ...
        for i in range(num_sections):
            ...
            if i == num_sections - 1:
                ...
                for source_model_idx in range(1, len(self.model_list)):
                    ...
                    if self.cache_alignment == "fuser" and self.cache_codec_list:
                        self.kv_cache_dict[self.base_model_idx][source_model_idx] = (
                            self._apply_source_cache_codec(
                                source_model_idx,
                                self.kv_cache_dict[self.base_model_idx][source_model_idx],
                            )
                        )
        ...
        if self.adaptive_quant_table is not None and total_adaptive_payload_bits is not None:
            ...
            self.last_adaptive_quant_table_metrics = {
                "task_ce_loss": float(task_loss.item()) if task_loss is not None else None,
                "alpha_by_layer_kv": (
                    result.alpha.detach().float().cpu().tolist()
                    if result is not None
                    else None
                ),
                "table_indices_by_layer_kv": (
                    result.table_indices.detach().cpu().tolist()
                    if result is not None
                    else None
                ),
                "estimated_payload_bits": float(
