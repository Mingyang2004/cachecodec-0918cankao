...
@register_model
@capture_init_args
class DirectConcatProjector(Projector):
    """Map a full Sharer KV cache directly into Receiver prefix geometry.

    This is the uncompressed Concat baseline: the complete Sharer K/V is the
    wire payload and only the geometry conversion is learned.  It never reads
    Receiver cache values, so it can be used as a prefix decoder.
    """
    ...
    def __init__(
        ...
        if min(
            ...
            raise ValueError("Direct concat dimensions must be positive.")
        if num_layers < 1:
            raise ValueError("Direct concat num_layers must be positive.")
        if not 0.0 <= dropout < 1.0:
            raise ValueError("Direct concat dropout must be in [0, 1).")
        ...
        source_width = self.sharer_num_kv_heads * self.sharer_head_dim
        receiver_width = self.receiver_num_kv_heads * self.receiver_head_dim

        def build_branch() -> nn.Sequential:
            layers: list[nn.Module] = []
            if self.num_layers == 1:
                layers.append(nn.Linear(source_width, receiver_width, dtype=dtype))
            else:
                layers.extend(
                    [
                        nn.Linear(source_width, self.hidden_dim, dtype=dtype),
                        nn.GELU(),
                    ]
                )
                if self.dropout:
                ...
                for _ in range(self.num_layers - 2):
                    layers.extend(
                        [
                            nn.Linear(self.hidden_dim, self.hidden_dim, dtype=dtype),
                            nn.GELU(),
                        ]
                    )
                    if self.dropout:
                        layers.append(nn.Dropout(self.dropout))
                layers.append(nn.Linear(self.hidden_dim, receiver_width, dtype=dtype))
            return nn.Sequential(*layers)

        self.decoder_k = build_branch()
        self.decoder_v = build_branch()

    def _decode_tensor(self, tensor: Tensor, decoder: nn.Module) -> Tensor:
        if tensor.ndim != 4:
            raise ValueError("Direct concat expects [B,H,S,D] tensors.")
        batch, heads, sequence_length, head_dim = tensor.shape
        if (heads, head_dim) != (self.sharer_num_kv_heads, self.sharer_head_dim):
            raise ValueError("Sharer KV geometry does not match direct concat.")
        flat = tensor.transpose(1, 2).contiguous().reshape(batch, sequence_length, -1)
        output = decoder(flat.to(dtype=next(decoder.parameters()).dtype))
    ...
    def encode(self, source_kv: tuple[Tensor, Tensor]) -> tuple[Tensor, Tensor]:
        ...
        if key.shape != value.shape:
            raise ValueError("Direct concat Sharer K/V shapes must match.")
        return key, value

    def decode_source(self, key: Tensor, value: Tensor) -> tuple[Tensor, Tensor]:
        return self._decode_tensor(key, self.decoder_k), self._decode_tensor(value, self.decoder_v)
    ...
    def forward(
        ...
        del target_kv
        return self.decode_source(*source_kv)
...
def load_projector(file_path: str, override_args: Optional[dict] = None) -> Projector:
    ...
    from rosetta.model import jcb  # noqa: F401
    from rosetta.model import split_channel_fusion  # noqa: F401
