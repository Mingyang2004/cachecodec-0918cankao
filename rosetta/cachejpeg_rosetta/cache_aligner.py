...
class ConcatCacheAligner:
    ...
    def decode(
        ...
        decoded_by_layer: dict[int, tuple[torch.Tensor, torch.Tensor]] = {}
        receiver_parameter = next(self.assets.base_model.parameters())
        receiver_device = receiver_parameter.device
        with torch.no_grad():
            for target_layer, _source_layer, projector_index in routing.routes:
                ...
                if key_latent.shape[1] != 1 or key_latent.shape != value_latent.shape:
                    raise ValueError("LCF-first pseudo K/V must have one head and matching shapes.")
                key_latent = key_latent.to(device=receiver_device, dtype=receiver_parameter.dtype)
                value_latent = value_latent.to(device=receiver_device, dtype=receiver_parameter.dtype)
                latent = torch.cat(
                    [key_latent.squeeze(1), value_latent.squeeze(1)], dim=-1
                )
                projector = self.assets.projector_list[projector_index].to(
                    device=receiver_device, dtype=receiver_parameter.dtype
                ).eval()
                key, value = projector.decode(latent)
                decoded_by_layer[target_layer] = (
                    key.to(device=receiver_device, dtype=receiver_parameter.dtype).contiguous(),
                    value.to(device=receiver_device, dtype=receiver_parameter.dtype).contiguous(),
                )
...
class RawConcatCacheAligner(ConcatCacheAligner):
    """Transport full Sharer KV and decode it with a direct prefix mapper."""
    ...
    def prepare_routing(self) -> LCFLatentRouting:
        ...
        try:
            ...
            raise ValueError("No sharer-to-receiver projector mapping is configured.") from error
        routes = []
        source_layer_count = int(self.assets.teacher_model.config.num_hidden_layers)
        source_width = None
        for target_layer, entry in layer_map.items():
            pairs = entry if isinstance(entry, list) else [entry]
            if len(pairs) != 1:
                raise ValueError("Raw concat requires one Sharer layer per Receiver layer.")
            source_layer, projector_index = self._normalize_pair(pairs[0])
            if not 0 <= source_layer < source_layer_count:
                raise ValueError(f"Raw concat route references invalid Sharer layer {source_layer}.")
            projector = self.assets.projector_list[projector_index]
            if not isinstance(projector, DirectConcatProjector):
                raise TypeError(
                    "Raw concat requires DirectConcatProjector checkpoints, "
                    f"got {projector.__class__.__name__}."
                )
            source_width = projector.sharer_num_kv_heads * projector.sharer_head_dim
            routes.append((int(target_layer), source_layer, projector_index))
        expected = set(range(int(self.assets.base_model.config.num_hidden_layers)))
        actual = {route[0] for route in routes}
        if actual != expected:
            raise ValueError(
                f"Raw concat is missing receiver layers: {sorted(expected - actual)}."
            )
        return LCFLatentRouting(
            routes=tuple(sorted(routes)),
            latent_dim=int(source_width or 0) * 2,
            sequence_length=0,
        )
    ...
    def encode(self, sharer_cache: Any):
        ...
        routing = self.prepare_routing()
        pseudo_cache = []
        for _target_layer, source_layer, _projector_index in routing.routes:
            key = sharer_cache.key_cache[source_layer].detach().cpu().contiguous()
            value = sharer_cache.value_cache[source_layer].detach().cpu().contiguous()
            pseudo_cache.append((key, value))
        sequence_length = int(pseudo_cache[0][0].shape[2]) if pseudo_cache else 0
        return tuple(pseudo_cache), LCFLatentRouting(
            routes=routing.routes,
            latent_dim=routing.latent_dim,
            sequence_length=sequence_length,
        )

    def decode(self, decoded_cache: Any, routing: LCFLatentRouting) -> DynamicCache:
        decoded_cache = self._to_dynamic_cache(decoded_cache)
        decoded_by_layer: dict[int, tuple[torch.Tensor, torch.Tensor]] = {}
        receiver_parameter = next(self.assets.base_model.parameters())
        receiver_device = receiver_parameter.device
        with torch.no_grad():
            for pseudo_index, (target_layer, _source_layer, projector_index) in enumerate(
                routing.routes
            ):
                key = decoded_cache.key_cache[pseudo_index].to(
                    device=receiver_device, dtype=receiver_parameter.dtype
                )
                value = decoded_cache.value_cache[pseudo_index].to(
                    device=receiver_device, dtype=receiver_parameter.dtype
                )
                projector = self.assets.projector_list[projector_index].to(
                    device=receiver_device, dtype=receiver_parameter.dtype
                ...
                if not isinstance(projector, DirectConcatProjector):
                    raise TypeError("Raw concat requires DirectConcatProjector checkpoints.")
                decoded_key, decoded_value = projector.decode_source(key, value)
                decoded_by_layer[target_layer] = (
                    decoded_key.to(device=receiver_device, dtype=receiver_parameter.dtype),
                    decoded_value.to(device=receiver_device, dtype=receiver_parameter.dtype),
                )
        ...
        self.last_alignment_stats = {
            "alignment_type": "concat",
            "concat_projector_type": "raw",
            "codec_order": "raw_transport_direct_concat",
            "rope_mode": "pre_rope",
            "prefix_tokens": int(prefix.key_cache[0].shape[2]),
            "routes": [list(route) for route in routing.routes],
        }
