...
class CacheJPEGRosettaEvalWrapper:
    ...
    def _encode_fixed_jcb_packet(self, latent_cache):
        ...
        for key, value in latent_cache:
            layers.append({
                "key": self.fixed_jcb_packet_codec.encode(key),
                "value": self.fixed_jcb_packet_codec.encode(value),
            })
    ...
    def _decode_fixed_jcb_packet(self, packet, device):
        ...
        return tuple(
            (
                self.fixed_jcb_packet_codec.decode(layer["key"], device=device),
                self.fixed_jcb_packet_codec.decode(layer["value"], device=device),
            )
