...
class LoadedRosettaAssets:
    ...
    cache_codec_list: list[Any] | None = None
...
class RosettaFuserBridge:
    ...
    def __init__(self, assets: LoadedRosettaAssets, adaptive_quant_table=None, cache_codec_list=None, cache_codec_packet=None):
        self.assets = assets
        self.adaptive_quant_table = adaptive_quant_table
        self.cache_codec_list = list(cache_codec_list or getattr(assets, "cache_codec_list", None) or [])
        self.cache_codec_packet = cache_codec_packet
        self.projector_dict = self._convert_dict_keys_to_ints(assets.projector_dict)
        self.last_fusion_stats: dict[str, Any] | None = None

    def fuse_teacher_cache_to_base(self, teacher_cache, base_seed_cache=None, *, apply_cache_codec: bool = True):
        """
        ...
        if apply_cache_codec and self.cache_codec_list:
            teacher_cache = self._apply_cache_codec(teacher_cache)
    ...
    def _apply_cache_codec(self, teacher_cache: DynamicCache) -> DynamicCache:
        """Reconstruct Sharer geometry before invoking the original C2C fuser."""
        source_model_idx = int(self.assets.teacher_model_idx)
        target_model_idx = int(self.assets.base_model_idx)
        layer_map = self.projector_dict.get(target_model_idx, {}).get(source_model_idx, {})
        restored = self._clone_dynamic_cache(teacher_cache)
        seen = set()
        for target_layer_idx, entry in layer_map.items():
            pairs = entry if isinstance(entry, list) else [entry]
            if not pairs:
                continue
            source_layer_idx, _ = self._normalize_pair(pairs[0])
            if source_layer_idx in seen:
                continue
            codec = self.cache_codec_list[min(int(target_layer_idx), len(self.cache_codec_list) - 1)]
            if not hasattr(codec, "decode_source"):
                raise TypeError("C2C cache codec must provide decode_source().")
            key = restored.key_cache[source_layer_idx]
            value = restored.value_cache[source_layer_idx]
            key_latent, value_latent = codec.encode((key, value))
            if self.cache_codec_packet is not None:
                key_latent = self.cache_codec_packet.decode(
                    self.cache_codec_packet.encode(key_latent.unsqueeze(1)),
                    device=key.device,
                ).squeeze(1)
                value_latent = self.cache_codec_packet.decode(
                    self.cache_codec_packet.encode(value_latent.unsqueeze(1)),
                    device=value.device,
                ).squeeze(1)
            key, value = codec.decode_source(key_latent, value_latent)
            restored.key_cache[source_layer_idx] = key.to(device=key.device, dtype=restored.key_cache[source_layer_idx].dtype)
            restored.value_cache[source_layer_idx] = value.to(device=value.device, dtype=restored.value_cache[source_layer_idx].dtype)
            seen.add(source_layer_idx)
        return restored
